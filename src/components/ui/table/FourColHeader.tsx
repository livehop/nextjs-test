...
const FourColHeader = ({ title1, title2, title3, title4 }: FourColHeaderProps) => {
    const { columnStore } = useStore();
    const { columns } = columnStore;

    return (
        <>
            {(isColumnVisible(columns, title1) || isColumnVisible(columns, title2) || isColumnVisible(columns, title3) || isColumnVisible(columns, title4)) && (
                <th
                    scope="col"
