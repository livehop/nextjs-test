...
const TestMe = () => {
    ...
    const { equipes, loadEquipes, loading, activeEquipe, setActiveEquipe } = kaizenStore;
    ...
    return (
        <Combobox as="div" value={activeEquipe} onChange={(e) => setActiveEquipe(e)}>
            <Combobox.Label className="block text-sm font-medium leading-6 text-gray-900">Equipe</Combobox.Label>
                    ...
                    <Combobox.Options className="absolute z-10 mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
                        {equipes.map((equipe) => (
                            ...
                            >
                                {({ active, selected }) => (
                                    <>
                                        <span className={classNames('block truncate', selected && 'font-semibold')}>{equipe.nomEquipe}</span>
