...
const Paging: React.FC<PagingProps> = ({ bottom = true }) => {
  ...
  const { kaizenDocuments } = kaizenStore;
  const { totalPages, searchKaizenDocuments } = searchStore;

  const getClassesForPage = (pageNum: number) => {
    if (pageNum === kaizenDocuments?.currentPage) {
      if (bottom)
        return "inline-flex items-center border-t-2 border-blue-500 px-4 pt-4 text-sm font-medium text-blue-600";
